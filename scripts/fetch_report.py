...
Companies are processed in parallel (ThreadPoolExecutor) since this is almost
entirely I/O-bound waiting on SEC — EdgarClient's rate limiter is thread-safe
and still enforces the same overall request pace across every worker.
"""
...
def process_company(
    ...
    """Runs the full per-company pipeline (filing lookup + XML parse) on
    whatever worker thread calls it. Returns everything the caller needs to
    merge rather than touching any shared state directly."""
    ...
    try:
        ...
        return 0, 0, transactions, errors
    ...
    return filings_found, filings_parsed, transactions, errors
...
def main() -> int:
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
        futures = {
            pool.submit(
                process_company, client, row["ticker"], row["cik"], prev_day, today_day
            ): row["ticker"]
            for row in constituents
        }
        for future in as_completed(futures):
            found, parsed, txns, errs = future.result()
            filings_found += found
